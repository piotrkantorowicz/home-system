...
public static class MealScheduleEndpoints
{
    ...
    public static IEndpointRouteBuilder MapMealScheduleEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", GetMealSchedule)
            ...
            .WithSummary("Get a household member's meal schedule configuration")
            .WithDescription("Returns the configured meal slots for the current user, or for the household member given by `personId`. Returns `null` body when no schedule has been set yet. 403 when `personId` is not a household member.");

        group.MapPut("/", UpdateMealSchedule)
            .WithName("UpdateMealSchedule")
            .WithSummary("Create or update meal schedule configuration")
            .WithDescription("Sets the meal slots for the current user, or for the managed member given by `personId` (Owner/Adult only). Replaces all existing slots.");
        ...
        CancellationToken ct)
    {
        MealScheduleConfigDto? result = await dispatcher.SendAsync<GetMealScheduleQuery, MealScheduleConfigDto?>(
            new GetMealScheduleQuery(GetPersonId(user), PersonalDataClaims.GetAuthSubject(user), personId), ct);
        ...
        CancellationToken ct)
    {
        ...
        await dispatcher.SendAsync(
            new UpdateMealScheduleCommand(GetPersonId(user), slots, PersonalDataClaims.GetAuthSubject(user), personId), ct);
