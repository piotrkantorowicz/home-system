...
- `rowFor(name)`: `getByRole('row', { name: new RegExp(name) })`
...
- **Selectors**: edit link is found via `row.getByRole('link', { name: /edit/i })`; Save button matches `/save|update/i` to handle the difference between create and edit copy
...
- **Selectors**: confirm dialog is `getByRole('dialog')` containing `/delete product/i`; the inner Delete button is matched by `/^delete$/i` (anchored regex avoids matching the "Delete Product" heading)
